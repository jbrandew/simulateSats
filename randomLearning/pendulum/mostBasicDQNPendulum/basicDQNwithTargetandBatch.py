...
class ReplayBuffer:
    ...
    def sample(self, n):
        ...
        done_batch = torch.tensor(np.array(done_mask_lst), dtype=torch.float)
...
class DQNAgent:
    def __init__(self):
        self.state_dim     = 3
        self.action_dim    = 17  #8 on each side, then 1 for 0 
        # 9개 행동 : -2, -1.5, -1.0, -0.5, 0.0, 0.5, 1.0, 1.5, 2.0
        self.lr            = 0.01
        #how much we value future rewards 
        self.gamma         = 0.98
        #how fast we are updating the network with respect to target network:
        self.tau           = 0.01
        #variables delineating our epsilon greedy exploration method 
        self.epsilon       = 1.0
        #increase decay rate because uhh...idk
        ...
        #create the target network, which slows down changes to network 
        self.Q_target = QNetwork(self.state_dim, self.action_dim, self.lr)
        #make the state of the parameters in the target network the same as the 
        #normal one 
    ...
    def choose_action(self, state):
        ...
        scalingFactor = ((self.action_dim-1)/2)/2
        ...
        if self.epsilon < random_number:
            ...
            with torch.no_grad():
                ...
                real_action = (action - (self.action_dim-1)/2) / scalingFactor
                maxQ_action_count = 1
        else:
             
            #seems like better way would be:  np.random.randint(low, high)
            action = np.random.choice([n for n in range(self.action_dim)])
            real_action = (action - (self.action_dim-1)/2) / scalingFactor # -2, -1.5, -1.0, -0.5, 0.0, 0.5, 1.0, 1.5, 2.0
    ...
    def calc_target(self, mini_batch):
        ...
        with torch.no_grad():
            ...
            #for each element. use the actual value, not the index of the max
            #(thats the [0] part. the max(1) is getting max along that axis)
            #use unsqueeze to expand dimension of data along that axis...
